/* ============================================================
   ...
   실행
     node staging/tests/i18n.test.mjs
   ============================================================ */
import { readFileSync, readdirSync } from "node:fs";
import { fileURLToPath } from "node:url";
import { dirname, join } from "node:path";

const STAGING = join(dirname(fileURLToPath(import.meta.url)), "..");
...
const files = readdirSync(STAGING)
  ...
  .filter((f) => KO.test(readFileSync(join(STAGING, f), "utf8")));

let missing = 0, checked = 0;
for (const f of files) {
  const s = readFileSync(join(STAGING, f), "utf8");
...
console.log("");
ok(`영어 화면에 한국어가 남는 문구가 없다 (파일 ${checked}개)`, missing === 0);
