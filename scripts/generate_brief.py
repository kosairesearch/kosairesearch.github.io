...
def _repair_prompt(brief, reasons):
    ...
    # 9/23 실전: '나열이다' 를 받고도 chips.p0 이 등락률 10개 그대로 나왔다(5→5개 링크 ·
    # 10→10개). "대여섯 개 안으로" 는 세어 보지 않으면 못 지킨다 — 지워야 할 개수를
    # 숫자로 박는다. Sonnet 5 는 글자 그대로 따르는 모델이라 이렇게 줘야 한다.
    todo = []
    for r in reasons:
        m = re.match(r"(\S+) 에 종목 링크 (\d+)개 · 등락률 (\d+)개 — 나열이다", r)
        if m:
            path, links, pcts = m.group(1), int(m.group(2)), int(m.group(3))
            todo.append(f"· [{path}.ko] 지금 종목 링크 {links}개 · 등락률 숫자 {pcts}개 → "
                        f"링크 {max(0, links - 4)}개, 등락률 숫자 {max(0, pcts - 5)}개를 지워야 통과한다"
                        f"(남기는 것: 링크 4개 이하 · 등락률 숫자 5개 이하). 영문(.en)도 같게.")
            continue
        m = re.match(r"(lead|summary)\.ko 에 등락률 (\d+)개 — 나열이다", r)
        if m:
            todo.append(f"· [{m.group(1)}.ko] 지금 등락률 숫자 {m.group(2)}개 → "
                        f"{max(0, int(m.group(2)) - 6)}개를 지워야 통과한다(6개 이하). 영문(.en)도 같게.")
    return (
        ...
        "· '나열이다' → 위 '통과 조건' 의 개수만큼 종목 링크와 등락률 숫자를 지운다. 등락률 숫자는"
        " 종목 것부터 지우고, 뺀 종목은 업종으로 묶어 숫자 없이 한 구절로 말한다('전자·부품 업종이"
        " 함께 올랐다'). 지수·업종 등락률은 대표 하나만 남긴다. 한국어·영문 같이.\n"
