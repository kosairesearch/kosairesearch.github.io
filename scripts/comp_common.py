...
/* 휴대폰 메뉴 — ☰ 를 누르면 헤더 아래로 펼쳐지는 목록. 링크 다섯 + 로그인(또는 계정) */
.menu .x{display:none} .nav.menu-open .menu .ham{display:none} .nav.menu-open .menu .x{display:block}
.mmenu{display:none;position:absolute;top:60px;left:0;right:0;background:var(--bg);border-top:1px solid var(--hair);border-bottom:1px solid var(--line);padding:6px 0 10px;z-index:56}
.nav.menu-open .mmenu{display:block}
.mmenu a,.mmenu button{display:block;width:100%;box-sizing:border-box;text-align:left;border:0;background:none;padding:14px var(--pad);font:500 18px/24px var(--font);color:var(--ink-72);text-decoration:none;cursor:pointer} .mmenu a.on{color:var(--ink);font-weight:600}
.mmenu .sep{height:1px;background:var(--hair);margin:6px var(--pad)} .mmenu .em{padding:10px var(--pad) 0;font:400 12px/16px var(--font);color:var(--ink-55);word-break:break-all}
.mback{display:none;position:fixed;left:0;right:0;top:60px;bottom:0;background:rgba(20,20,20,.16);z-index:55} .nav.menu-open .mback{display:block}
...
<div class="mmenu" id="mmenu">{mlinks}<div class="sep"></div><div id="mauth"><a href="/Login.html">로그인</a></div></div>
<div class="mback" id="mback"></div></nav>'''
