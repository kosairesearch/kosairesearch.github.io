...
.wl svg{width:18px;height:18px;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linejoin:round} .wl.on{color:var(--ink)} .wl.on svg{fill:currentColor}
/* 쪽 넘기기 */
...
BODY = '''<main class="wrap">
  ...
  <div class="rl" id="rl">
    <div class="rl-head"><span>NO.</span><span>종목</span><span>리포트 제목</span><span>현재가</span><span>작성일</span><span>북마크</span></div>
    <div id="rows"></div>
  </div>
...
JS = r'''(function(){
  ...
  var bmSvg='<svg viewBox="0 0 24 24"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg>';
  function rowHtml(r,i){var c=r.change||0,t=repTitle(r.ticker),d=repDate(r.ticker);
    return '<a class="row" href="/stock.html?ticker='+r.ticker+'" data-tk="'+r.ticker+'"><span class="rk">'+(i+1)+'</span>'
      ...
      +'<button type="button" class="wl'+(isW(r.ticker)?' on':'')+'" data-wl="'+r.ticker+'" aria-label="관심종목 추가/해제">'+bmSvg+'</button></a>'}
  ...
  rowsEl.addEventListener('click',function(e){var b=e.target.closest('.wl');if(!b)return;e.preventDefault();e.stopPropagation();var tk=b.dataset.wl;if(isW(tk))delete WL[tk];else WL[tk]=1;b.classList.toggle('on',isW(tk))});
