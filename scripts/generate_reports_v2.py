...
SCHEMA_V2 = """{
  ...
  "valuation_comment": {"ko": "밸류에이션 해설 4~6문장. ★현재 PER·PBR·배당수익률·현재가·시가총액은 주가 따라 매일 바뀌므로 '정확한 수치'를 문장에 쓰지 말 것(그 값은 화면 카드가 실시간 표시). 대신 수준을 관계로 서술 — 예: 과거 거래 밴드 상단/하단, 업종 평균 상회/하회, 순자산 대비 프리미엄/할인. ★ROE·EPS·BPS를 언급할 땐 반드시 제공된 valuation의 헤드라인 값(roe_ttm, eps, bps = 사용자 화면 카드와 동일)만 사용할 것. 특정 연도 연간 ROE·EPS 수치는 valuation_comment에 쓰지 말 것(연간 추이 해설은 earnings 섹션이 담당). 다년 변화가 필요하면 '적자→흑자 전환', '이익 회복' 같은 방향으로만 표현. 'TTM'·'후행' 등 전문 용어는 표면에 쓰지 말 것. '비싸다/싸다' 단정·권유 금지, 사실 비교만", "en": "..."},
...
3. earnings 섹션은 제공된 연간·분기 실적 수치(과거 확정치라 안 변함)를 구체적으로 인용·해석하세요. **valuation_comment 에서는 '현재 PER·PBR·배당수익률·현재가·시가총액'의 정확한 수치를 문장에 쓰지 마세요** — 이 값들은 주가 따라 매일 바뀌고 화면 카드가 실시간으로 표시합니다. 대신 그 수준을 '관계'로 서술하세요(예: "과거 거래 밴드(약 10~20배)의 상단을 웃돈다", "배당수익률은 업종 평균을 밑도는 편", "순자산 대비 프리미엄이 큰 구간"). 과거 PER 밴드, DPS, 다년 실적 추세는 인용해도 됩니다. **단 헤드라인 ROE·EPS·BPS는 반드시 [확정 재무]의 valuation.roe_ttm·eps·bps(사용자 카드와 동일한 값)만 쓰고, 연도별 ROE·EPS 수치는 valuation_comment에 넣지 마세요**(연간 실적 해설은 earnings 섹션에서 하세요). 다년 변화가 필요하면 "적자에서 흑자로 전환", "이익 회복" 처럼 방향으로만 표현하세요. 'TTM'·'선행/후행' 같은 용어와 '비싸다/싸다' 단정·매수/매도 권유는 금지.
