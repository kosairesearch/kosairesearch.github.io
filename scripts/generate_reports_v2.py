...
import number_spacing       # 저장 직전 금액 표기 통일(79조3,187억원 → 79조 3,187억원)
...
5-2. **금액 표기** — 한글 맞춤법 제44항에 따라 수는 '만' 단위로 띄어 쓴다. "79조 3,187억원"(O), "79조3,187억원"(X). '원'은 붙여 쓴다: "1,507억원"(O), "1,507억 원"(X). 한 리포트 안에서 표기를 섞지 말 것.
...
def collect(cl, as_of, state):
    ...
    for result in cl.messages.batches.results(batch_id):
        ...
        try:
            ...
            # 금액 표기 통일 — 한글 맞춤법 제44항(수는 '만' 단위로 띄어 쓴다).
            # 프롬프트에 규칙을 적어도 매번 같게 나오지 않는다. 2,564개를 세니
            ...
            try:
                ...
                log(f"  · ({tk} 금액 표기 정리 실패: {type(e).__name__}: {e})")
