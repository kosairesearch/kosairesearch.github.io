...
SCHEMA_V2 = """{
  ...
  "valuation_comment": {"ko": "밸류에이션 해설 4~6문장. ★현재 PER·PBR·배당수익률·현재가·시가총액은 주가 따라 매일 바뀌므로 '정확한 수치'를 문장에 쓰지 말 것(그 값은 화면 카드가 실시간 표시). 대신 수준을 관계로 서술 — 예: 과거 거래 밴드 상단/하단, 업종 평균 상회/하회, 순자산 대비 프리미엄/할인. ★ROE·EPS·BPS를 언급할 땐 반드시 제공된 valuation의 헤드라인 값(roe_ttm, eps, bps = 사용자 화면 카드와 동일)을 사용할 것. 특정 연도 연간 수치를 쓰려면 'YYYY년 연간 기준'이라고 명시해 카드값과 모순돼 보이지 않게 하라(예: 카드 ROE와 연간 ROE가 다를 수 있음). 'TTM'·'후행' 등 전문 용어는 표면에 쓰지 말 것. '비싸다/싸다' 단정·권유 금지, 사실 비교만", "en": "..."},
...
3. earnings 섹션은 제공된 연간·분기 실적 수치(과거 확정치라 안 변함)를 구체적으로 인용·해석하세요. **valuation_comment 에서는 '현재 PER·PBR·배당수익률·현재가·시가총액'의 정확한 수치를 문장에 쓰지 마세요** — 이 값들은 주가 따라 매일 바뀌고 화면 카드가 실시간으로 표시합니다. 대신 그 수준을 '관계'로 서술하세요(예: "과거 거래 밴드(약 10~20배)의 상단을 웃돈다", "배당수익률은 업종 평균을 밑도는 편", "순자산 대비 프리미엄이 큰 구간"). 잘 변하지 않는 숫자—과거 PER 밴드, EPS·BPS·ROE·DPS, 다년 실적—는 본문에 인용해도 됩니다. **단 헤드라인 ROE·EPS·BPS를 언급하면 반드시 [확정 재무]의 valuation.roe_ttm·eps·bps(사용자 카드와 동일한 값)를 쓰세요.** 특정 연도 연간 ROE·EPS를 쓰려면 "YYYY년 연간 기준"이라고 분명히 밝혀 카드값과 충돌해 보이지 않게 하세요(연간 수치와 카드 헤드라인 값이 다를 수 있음). 'TTM'·'선행/후행' 같은 용어와 '비싸다/싸다' 단정·매수/매도 권유는 금지.
