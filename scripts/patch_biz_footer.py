...
# 라벨은 '전자우편'(전자상거래법 제10조의 법령 용어)이 아니라 '이메일'로 쓴다.
# 법이 요구하는 것은 정보를 표시하는 것이지 라벨의 글자가 아니고, 이 사이트의
# 문의 화면이 이미 '이메일'로 쓰고 있어서 푸터만 다른 말을 쓰면 안에서 어긋난다.
# '연락처'는 보통 전화번호를 가리키므로 이메일만 있는 지금은 맞지 않는다.
#
