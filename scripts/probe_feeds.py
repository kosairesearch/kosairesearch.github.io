...
봐야 한다. 이 스크립트는 그 '찔러 보기'다 — 돈이 들지 않는다(API 호출 없음).
    ...
    python3 scripts/probe_feeds.py              # 러너에서

(scripts/probe_sources.py 는 9월 초 소스를 고를 때 쓴 다른 판정기다 — docs/brief_design.md ·
 probe_sources.yml. 9/22 에 이 파일을 같은 이름으로 만들어 그걸 덮어썼다가 9/26 에 되살렸다.)
