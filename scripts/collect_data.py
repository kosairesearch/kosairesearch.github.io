...
NAME_EN_OVERRIDE = {
    ...
    "0009K0": "AimedBio",
    "0011T0": "Chaevi",
    "0120G0": "Samyang Biopharm",
    "0082N0": "Kanaph Therapeutics",
    "0001A0": "Deokyang Energen",
    "0030R0": "Daishin Value REIT",
    "0008Z0": "SNSYS",
    "0015G0": "Green Optics",
    "0088M0": "Mezoo",
    "0007C0": "Acryl",
...
_PREF_RE = re.compile(r"\d*우[A-Z]?(\([가-힣]+\))?$")


def _is_pref_or_spac(name):
    n = str(name or "")
    return bool(_PREF_RE.search(n)) or ("스팩" in n)
...
def collect_krx_cache(date):
    ...
    for _, row in df.iterrows():
        ...
        nm = str(row.get("Name", ""))
        # 우선주(우/우B/우C/우(전환) 등)·스팩 제외 — 회사 리서치 universe에서 노이즈
        if _is_pref_or_spac(nm):
            skipped += 1
            continue
        try:
