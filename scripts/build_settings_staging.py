...
#mount .ks-nav button{width:auto;flex:none;border:0;border-radius:0;background:none;padding:0;font:500 13px/40px var(--font);color:var(--ink-62);white-space:nowrap;transition:color .12s} #mount .ks-nav button:hover{background:none;color:var(--ink)}
...
#mount .ks-h{margin:26px 0 12px;font:600 12px/16px var(--font);color:var(--ink-62);letter-spacing:.06em;text-transform:none}
#mount .ks-panel>:first-child>.ks-sec:first-child>.ks-h,#mount .ks-panel>.ks-sec:first-child>.ks-h{display:none}  /* 각 칸은 div > .ks-sec 로 온다. 첫 칸 머리(일반·구독…)는 탭 이름과 같아 겹친다 — 뒤 칸(결제 내역)의 머리만 남긴다 */
#mount .ks-row{display:flex;align-items:center;justify-content:space-between;gap:24px;padding:18px 0;border-bottom:1px solid var(--hair)} #mount .ks-row .ks-lab{font:500 15px/22px var(--font);color:var(--ink)} #mount .ks-row .ks-sub{margin-top:4px;font:400 13px/20px var(--font);color:var(--ink-62);max-width:360px}
#mount .ks-seg{width:auto;border:0;border-radius:0;background:none;display:flex;gap:18px;flex:none} #mount .ks-seg button{flex:none;padding:0;font:500 13px/32px var(--font);color:var(--ink-62);background:none;border-radius:0;box-shadow:none} #mount .ks-seg button[aria-pressed="true"]{background:none;color:var(--ink);font-weight:600;box-shadow:inset 0 -2px 0 var(--ink)}
#mount .ks-sw{width:40px;height:22px;border-radius:999px;border:1px solid var(--line);background:transparent} #mount .ks-sw::after{top:3px;left:3px;width:14px;height:14px;background:var(--ink-62);box-shadow:none} #mount .ks-sw[aria-checked="true"]{background:var(--ink);border-color:var(--ink)} #mount .ks-sw[aria-checked="true"]::after{background:var(--bg);transform:translateX(18px)}
#mount .ks-kv{grid-template-columns:120px 1fr;gap:12px 16px;padding:0;font:400 15px/22px var(--font)} #mount .ks-kv dt{color:var(--ink-62)} #mount .ks-kv dd{color:var(--ink)}
...
#mount .ks-hist{margin-top:4px} #mount .ks-hist table{font:400 14px/20px var(--font)} #mount .ks-hist th{padding:0 12px 10px 0;font:500 12px/16px var(--font);color:var(--ink-62);border-bottom:1px solid var(--line)} #mount .ks-hist td{padding:11px 12px 11px 0;border-top:1px solid var(--hair)}
/* 확인 창 — body 에 붙는다 */
