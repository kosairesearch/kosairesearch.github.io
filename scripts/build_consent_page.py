...
MAIN = '''<main>
  <div class="wrap">
    <div class="head">
      ...
      <h1 id="ttl">약관 동의</h1>
      <p id="lede">가입을 완료하시려면 아래 항목에 동의하여 주시기 바랍니다.</p>
    </div>
...
DICT = '''if(window.KOSi18n) KOSi18n.register({
  ...
  "가입 완료":"Almost done",
  "메일 인증만 남았습니다.":"Just one more step — verify your email.",
  "인증 메일을 보내 드리지 못했습니다":"Could not send the verification email",
  ...
  "보내지 못했습니다. 잠시 후 다시 시도하여 주시기 바랍니다.":"Could not send. Please try again in a moment.",
...
/* 우리가 일부러 로그아웃하거나 계정을 지우는 동안 아래 리스너가 끼어들지
   ...
   '인증 메일을 보내 드렸습니다' 를 띄우려던 순간 화면이 로그인으로 넘어갔다.
...
document.getElementById('agreeBtn').addEventListener('click', async () => {
  ...
  try{
    ...
    if(/not-found|user-not-found|unauthenticated|permission-denied/.test(code)){
      errBox.textContent = T('가입이 만료되어 계정이 삭제되었습니다. 처음부터 다시 가입하여 주시기 바랍니다.') + ' ';
      const a = document.createElement('a');
    ...
    showErr(T('동의를 저장하지 못했습니다. 잠시 후 다시 시도하여 주시기 바랍니다.'));
  }
});
...
function showVerifySent(mail, sent){
  ...
  if(sub){ sub.textContent = T('메일 인증만 남았습니다.'); }
  ...
  b.textContent = T('인증 메일을 보내 드렸습니다');
  if(!sent){ b.textContent = T('인증 메일을 보내 드리지 못했습니다'); }
  ...
  again.addEventListener('click', async () => {
    ...
    try{
      ...
      again.textContent = T('보내지 못했습니다. 잠시 후 다시 시도하여 주시기 바랍니다.');
    }
  });
