...
# ── 프롬프트 ──────────────────────────────────────────────────────────
SCHEMA_DESC = """{
  "title":     {"ko": "리포트 헤드라인(핵심 관점, 약 12~28자 범위에서 내용에 맞게 자유로운 길이로. 매수/매도 권유 금지)", "en": "headline"},
...
7. 제목(title)은 핵심 관점을 담은 헤드라인으로, **종목 내용에 맞춰 약 12~28자 범위에서 자유로운 길이**로 작성하세요(짧게 핵심만일 수도, 조금 더 구체적일 수도 있음 — 매번 비슷한 길이로 획일화하지 말 것). 예: "성장과 수익성의 괴리"(짧게) 또는 "MLCC·FC-BGA가 여는 AI 부품 토털 솔루션"(길게). 매수/매도·목표주가 등 투자권유 표현 금지.
