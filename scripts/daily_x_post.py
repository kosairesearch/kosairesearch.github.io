...
def draft(brief, snap):
    ...
    sys_p = (
        ...
        "[출력 구조] 반드시 아래 6개 ■ 섹션 헤더를 모두 그대로 포함한다(생략·통합·이름변경 금지). "
        "섹션 사이에는 빈 줄을 둔다. 각 서술 섹션은 1~2문장으로 짧게, 개조식 섹션은 '- ' 항목으로.\n"
        "첫 줄: '{종목명} ({코드}) · {섹터} · 당일 {등락}%' 형식의 제목 한 줄.\n"
...
def format_report(text):
    ...
    for raw in text.split("\n"):
        s = raw.strip()
        if not s:
            out.append("")                       # 빈 줄(섹션 구분) 유지
            continue
        if s[0] in "■-·•" or s.startswith("- "):  # 헤더/개조식/제목·불릿 → 한 줄 유지
            out.append(s)
            continue
        sents = _sentences(s)                     # 서술형 → 문장별 줄바꿈
        out.extend(sents if sents else [s])
    t = re.sub(r"\n{3,}", "\n\n", "\n".join(out)).strip()
    return t
