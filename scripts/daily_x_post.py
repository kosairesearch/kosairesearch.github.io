...
def draft(brief, snap):
    ...
    sys_p = (
        ...
        "[출력 구조] 아래 섹션 헤더(■)를 그대로 쓰고, 섹션 사이에는 빈 줄을 둔다. "
        "각 섹션은 밀도 있는 2~3문장 또는 '- ' 개조식 항목으로.\n"
        ...
        "[문체] 전문 애널리스트의 문어체 '~다' 종결. 간결하되 아마추어처럼 뚝뚝 끊지 말고, 한 문장에 "
        "하나의 논지를 밀도 있게 담아라. 금융 용어는 정확히(P-CAB·원외처방액·마일스톤·부채비율 등 그대로). "
...
def main():
    ...
    tk = stock["ticker"]
    # 리포트는 모델이 만든 섹션 구조(■ 헤더 + 문단)를 존중한다. 과도한 빈 줄·줄끝 공백만 정리.
    msg = re.sub(r"[ \t]+\n", "\n", d["ko"].strip())
    msg = re.sub(r"\n{3,}", "\n\n", msg).rstrip()
