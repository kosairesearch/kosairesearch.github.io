...
# 402 + 76 ≠ 446. 처음 오고 그 주에 또 온 사람은 양쪽에 다 들어간다.
ok("처음+다시 온 이 방문자가 아니라고 적는다", "양쪽에 다 들어갑니다" in m,
   m[m.find("다시 온"):][:160])
...
# 자료의 가장 최근 주는 9/7(FW) 이다. 그래야 8/31 코호트의 '1주 뒤'(9/7~9/13)가 통째로 지난 것이 된다.
RET = {"weeks": H[:-1] + [FW], "retention": [
...
# 2026-09-14 사고. 9/7 코호트의 '1주 뒤' 는 9/14~9/20 인데 9/14 아침에
# 받으니 몇 시간치(4명)만 들어 있었고, 그것을 "1%" 라고 찍었다.
RET2 = {"weeks": H[:-1] + [FW], "retention": RET["retention"][:2] + [
    {"week": "2026-09-07", "size": 396, "back": {"1": 4}}]}
co2 = M.latest_cohort(RET2)
eq("가장 최근 주의 코호트는 '1주 뒤' 가 덜 찼으니 뺀다",
   [c[0] for c in co2], ["2026-08-24", "2026-08-31"])
...
# 이미 되어 있었다. 모델은 저장소를 못 보므로 재료로 알려 줘야 한다.
_f = M.facts_text(GOOD)
ok("이미 한 일 칸이 있다", "[이미 해 놓은 일" in _f, _f[:200])
ok("서치콘솔이 들어 있다", "서치콘솔" in _f and "사이트맵" in _f)
ok("발자국 차단이 들어 있다", "발자국 차단" in _f and "noga" in _f)
ok("다시 제안하지 말라고 한다", "제안하지 마라" in _f)
ok("등록했는데 안 들어오는 것과 구분하라고 한다",
   "등록했는데도 안 들어온다" in _f, _f[_f.find("이미 해 놓은"):][:400])
ok("지시문에도 같은 말이 있다", "이미 해 놓은 일" in M.PROMPT)

print("\n▣ 같은 주를 다시 쓰면 제안을 올리지 않는다")
# 2026-09-15. 옛 주를 채우느라 9/7 주를 세 번 돌렸더니 대장에 exp_4·exp_5 가
# 연달아 생겼다. 숫자는 그대로인데 제안만 늘었다.
box = {"items": [{"week": "2026-09-07", "to": "2026-09-13", "text": "…"}]}
ok("전에 쓴 주는 '다시 쓰는 것'", M.already_reported("2026-09-07", box))
