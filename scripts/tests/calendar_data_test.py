...
print("\n── 수동 등록이 말라붙는 것 ──")
# 예전에는 이것을 경보로 잡았다. 그때는 일정의 절반이 이 파일이었기
# 때문이다. 지금은 FOMC·미국 지표·금통위가 자동으로 들어오므로 이 파일이
# 비어도 달력은 채워진다. 알리되 울리지는 않는다 — 평일마다 울리는
# 경보는 곧 아무도 안 본다. 달력이 정말 비는 것은 아래 '앞으로 N일'이 잡는다.
rows, h = C.manual()
ok("말라 있다는 사실은 적는다", "앞으로 잡힌 것이 없다" in h["note"], h["note"])
ok("마지막 등록일과 며칠 지났는지를 적는다",
   "2026-08-27" in h["note"] and "일 지남" in h["note"], h["note"])
ok("그렇다고 경보를 울리지는 않는다", h["ok"], h["note"])
