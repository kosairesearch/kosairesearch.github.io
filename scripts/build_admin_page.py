    ...
    <div class="adm" id="admBody" hidden>
      ...
      <section class="adm-card card glass" id="ncSec" hidden>
        ...
        <div class="adm-act">
          ...
          <button type="button" class="btn" id="recheckBtn">2년 재확인 대상 확인</button>
...
DICT = '''if(window.KOSi18n) KOSi18n.register({
  ...
  "2년 재확인 대상 확인":"Preview biennial reconfirmation",
  "2년이 지난 마케팅 동의가 없습니다.":"No marketing consent is older than two years.",
  "이 대상입니다. 기한 이레 전부터 자동으로 나갑니다.":" due. These are sent automatically from seven days before the deadline.",
  "주소 없음":"no address",
...
/* 2년 재확인 — 미리 보기.
   ...
   실제 발송은 매일 12:40 에 서버가 알아서 한다(marketingRecheck). 기한이
   계정마다 다르므로 날마다 보고 기한 이레 전부터 내보낸다.
   이 버튼은 '그날 무엇이 나갈지' 를 지금 눈으로 보기 위한 것이다. 첫
   대상이 나오는 것은 2028년이라, 그때까지 한 번도 못 돌려 보고 두면
   그날 처음 터진다.

   여기서는 보내지 않는다. dryRun 을 끄는 길을 화면에 두지 않았다 —
   자동으로 나가는 메일을 사람이 손으로 또 보낼 이유가 없고, 실수로
   두 번 나가는 쪽이 훨씬 나쁘다. */
$('#recheckBtn').addEventListener('click', async () => {
  ...
  note.textContent = ' ' + T('확인하는 중…');
  try{
    const r = await call('adminMarketingRecheck', { dryRun: true });
    const rows = r.targets || [];
    if(!r.due){
      note.textContent = ' ' + T('2년이 지난 마케팅 동의가 없습니다.');
    }else{
      note.textContent = ' ' + countText(r.due) + T('이 대상입니다. 기한 이레 전부터 자동으로 나갑니다.')
        + (r.noEmail ? ' · ' + T('주소 없음') + ' ' + r.noEmail : '')
        + (rows.length ? ' — ' + rows.map(t => t.email).join(', ') : '');
    }
  }catch(e){
    note.textContent = ' ' + ((e && e.message) || T('불러오지 못했습니다.'));
  }
