...
DICT = '''if(window.KOSi18n) KOSi18n.register({
  ...
  "이 대상입니다. 매달 1일에 자동으로 나갑니다.":" due. These are sent automatically on the 1st of each month.",
  "주소 없음":"no address",
...
/* 2년 재확인 — 미리 보기.
   ...
   실제 발송은 매달 1일 12:40 에 서버가 알아서 한다(marketingRecheck).
...
$('#recheckBtn').addEventListener('click', async () => {
  ...
  try{
    ...
    if(!r.due){
      ...
      note.textContent = ' ' + countText(r.due) + T('이 대상입니다. 매달 1일에 자동으로 나갑니다.')
