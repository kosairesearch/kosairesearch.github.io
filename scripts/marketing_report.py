...
def metrics_block(doc, today=None):
    ...
    # 업계 표준 — 크기(MAU·WAU·DAU)와 습관(DAU÷MAU). 사장이 "DAU·MAU 가 뭐냐" 고
    # 물었고(2026-09-15), 이어서 "내가 아는 단어니까 단어 그대로 써" 라고 했다.
    # 그래서 이 넷은 풀어 쓰지 않는다. 마케팅 회사·투자자가 제일 먼저 묻는 숫자다.
    if cur.get("mau28") is not None:
        L.append("■ MAU · WAU · DAU (지난 28일 · 지난 7일 · 하루 평균에 온 사람)")
        L.append(_line("MAU", cur.get("mau28"), (prev or {}).get("mau28"), "명"))
        if cur.get("wau7") is not None:
            L.append(_line("WAU", cur.get("wau7"), (prev or {}).get("wau7"), "명"))
            L.append("     └ 방문자와 거의 같습니다. 활동한 사람만 세서 몇 명 적습니다.")
        L.append(_line("DAU", cur.get("dauAvg"), (prev or {}).get("dauAvg"), "명"))
        st, stb = cur.get("stickiness"), (prev or {}).get("stickiness")
        if st is not None:
            tail = f"  {_pp(st, stb)}  (앞주 {stb:.1f}%)" if stb is not None else ""
            L.append(f"  {_pad('DAU/MAU', 16)}{st:.1f}%{tail}")
            L.append("     └ 한 달에 온 사람이 하루에 몇 % 오나. 10%면 한 달에 3일쯤 온다는 뜻입니다.")
        L.append("")
...
# 이미 해 놓은 일. 모델은 저장소를 못 보므로, 안 알려 주면 이미 되어 있는
# 것을 "하세요" 라고 제안한다. 2026-09-14 에 실제로 그랬다 — 구글·네이버
# 서치콘솔 등록과 내부 트래픽 제외를 둘 다 제안했는데 둘 다 되어 있었고,
# 사장이 "어제 했는데 까먹냐" 고 했다.
#
# 새로 무엇을 해 놓으면 여기에 한 줄 추가한다. 여기 없는 것은 모델이
# 아직 안 된 것으로 보고 제안한다.
DONE = [
    ("네이버 서치어드바이저 등록", "index.html 에 인증 메타가 있다"),
    ("구글 서치콘솔 등록", "사장이 등록해 두었다"),
    ("사이트맵 제출", "sitemap.xml 이 있고 robots.txt 가 가리킨다"),
    ("우리 발자국 차단", "2026-09-13 부터 /staging/·/Admin.html·?noga=1 은"
                        " 아예 안 보낸다. IP 필터보다 나은 방식이다"),
    # 2026-09-15·09-21 에 보고서가 아래 둘을 새 실험이라고 냈다(exp_1·exp_5·exp_8).
    # 코드를 열어 보니 이미 있었다. 다시 내지 않게 여기 적어 둔다.
    ("리포트 상단 관심종목 버튼", "종목 이름 옆에 '워치리스트 추가' 버튼이 이미 있다"
                             " (stock.html #watchBtn)"),
    ("리포트 하단 관련 종목·리포트 링크", "리포트 맨 아래에 같은 업종 종목 12개가 카드로"
                                  " 링크돼 있다 (stock.html renderPeers). 안 눌리는 것이"
                                  " 문제지 없는 것이 아니다"),
    ("랜딩페이지", "첫 화면(/, index.html)이 랜딩페이지다. 리포트 홈(/Home.html)은 따로 있다"),
    ("모닝 브리핑", "매일 아침 brief.html 에 새 글이 나온다. 안 알려진 것이 문제다"),
    ("대시보드", "사장이 Looker Studio 대시보드와 GA4 대시보드를 2026-09-15~16 에 만들었다"),
]
...
def facts_text(doc):
    ...
    sw = swing_pct(weeks)
    L.append("\n[이미 해 놓은 일 — 이것을 '하세요' 라고 제안하지 마라]")
    for what, how in DONE:
        L.append(f"  · {what} — {how}")
    L.append("  · 숫자가 나쁘다고 해서 안 한 것이 아니다. 구글에서 1회만 들어온다는"
             " 것은 '등록을 안 했다' 가 아니라 '등록했는데도 안 들어온다' 이다."
             " 둘은 할 일이 완전히 다르다.")
    L.append("  · 우리 발자국 차단은 2026-09-13(일)에 들어갔다. 그 주까지는 섞여"
             " 있고 9/14 주부터 깨끗하다. 9/14 주에 계정·관리·시험 조회가 확"
             " 줄면 성과 변화가 아니라 이 차단이 걸린 것이다.")
...
■ 무슨 일이 있었나
  세 줄에서 다섯 줄. 각 줄은 '· ' 로 시작하고 두 문장을 넘기지 마라.
  문단으로 쓰지 마라 — 휴대폰에서 글자 벽처럼 보여서 안 읽힌다.
...
■ 다음 주에 할 것
  ...
  '무엇을' 은 오늘 할 수 있는 한 걸음이어야 한다. "SEO를 강화하자",
  "콘텐츠를 늘리자" 는 어느 사이트에나 붙는 말이라 아무 쓸모가 없다.
  위 [이미 해 놓은 일] 에 있는 것을 다시 시키지 마라. 그것 때문에 숫자가
  나쁜 것처럼 보이면, '하세요' 가 아니라 "해 놨는데도 안 된다" 를 짚어라.
